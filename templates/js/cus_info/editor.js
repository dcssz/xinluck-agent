...
function change_save_btn_area(area_id){
	...
	if(area_id == 'wallet-info-area' || area_id == 'bank-info-area')
		save_btn = "";
...
/*function change_bank_province(){
	...
	requestJSON("cus_info_editor_op.php", "pdisplay=change_bank_province", "bank_province=" + bank_province);
}*/
...
function show_add_bank_info_div(){
	...
	if(!$("#add-bank-info-div").is(':visible')){
		close_layer({type: 1});
		requestJSON("cus_info_editor_op", "pdisplay=show_add_bank_info_div", "edit_cus_id=" + edit_cus_id);	
	}
}
...
function bank_ver_div_change_bank_area(bank_area){
	var target_e = $('#bank-ver-div #bank-name-select');
	var hidden_bank_name = $('#bank-ver-div #hidden-bank-name').val();
	target_e.html('');
...
function save_add_bank_info(){
	var edit_cus_id = $('#edit-cus-id').val();
	requestJSON("cus_info_editor_op", "pdisplay=save_add_bank_info", "edit_cus_id=" + edit_cus_id, "#add-bank-info-form");
}

function change_bank_info_status(edit_info_id, edit_status){
	var edit_cus_id = $('#edit-cus-id').val();
	requestJSON("cus_info_editor_op", "pdisplay=change_bank_info_status", "edit_cus_id=" + edit_cus_id + "&edit_info_id=" + edit_info_id + "&edit_status=" + edit_status);
}

function delete_bank_info(edit_info_id){
	var edit_cus_id = $('#edit-cus-id').val();
	requestJSON("cus_info_editor_op", "pdisplay=delete_bank_info", "edit_cus_id=" + edit_cus_id + "&edit_info_id=" + edit_info_id);
}

//=== 點數相關 === //
function reload_quota(){
	request_main_quota();
	$('#wallet-info-area .gstore-quota').each(function(){
		var this_e = $(this);
		var game_store = this_e.attr('gstore');
		request_gstore_quota(game_store);
	});
}

function request_main_quota(){
	$('#main-quota').attr('status', '0').html('Loading..');
	$('#total-quota').attr('status', '0').html('Loading..');
	var edit_cus_id = $("#edit-cus-id").val();
	requestJSON("cus_info_editor_op", "pdisplay=request_main_quota", "edit_cus_id=" + edit_cus_id);
}

function request_gstore_quota(game_store){
	$('#gstore-' + game_store + '-quota').attr('status', '0').html('Loading..');
	var edit_cus_id = $("#edit-cus-id").val(); 
	requestJSON("cus_info_editor_op", "pdisplay=request_gstore_quota", "edit_cus_id=" + edit_cus_id + "&game_store=" + game_store);
}

function show_quota(target_id, status, quota){
	if(status == 2){
		quota = "<font class='warning-txt'>" + change_lang_txt({"org_txt" : "讀取異常"}) + "</font>";
	}else if(status == 3){
		quota = "<font class='warning-txt'>" + change_lang_txt({"org_txt" : "維護中"}) + "</font>";
	}
	$("#" + target_id).attr('status', status).html(quota);
	cmp_total_quota();
}

function show_ext_game_cus_userid(target_id, ext_game_cus_userid){
	if(ext_game_cus_userid == "")
		ext_game_cus_userid = "<font class='warning-txt'>" + change_lang_txt({"org_txt" : "尚未註冊"}) + "</font>";
		
	$("#" + target_id).html(ext_game_cus_userid);
}

function cmp_total_quota(){
	var can_update = 1;
	var total_quota = 0;
	$('#wallet-info-area .quota').not('.total-quota').each(function(){
		var this_e = $(this);
		//如有任一個讀取中 就不能計算總額度
		if(this_e.attr('status') == 0){
			can_update = -1;
			return;
		}
		
		if(this_e.attr('status') == 1)
			total_quota += parseFloat(this_e.text());
	});
	
	if(can_update == 1){
		$('#total-quota').html(floor_dec(total_quota, 2));
	}
}

//點數全數轉回主帳戶
function return_to_main_account(){
	if(return_to_main_account_flag == 0){
		...
		if (bool){
			page_content_mask_show();
			var edit_cus_id = $("#edit-cus-id").val();
			requestJSON("cus_info_editor_op", "pdisplay=return_to_main_account", "edit_cus_id=" + edit_cus_id, "");
		}else{
...
//更新指定帳戶的API帳號
function update_ext_game_cus_userid(id, ext_game_cus_userid){
	$("#" + id).text(ext_game_cus_userid);
}

//更新指定帳戶的金額
function update_game_store_ticket_credits(id, ticket_credits){
	$("#" + id).text(ticket_credits);
	if(isNaN(ticket_credits)){
		$("#" + id).addClass("red-txt");
	}else{
		$("#" + id).removeClass("red-txt");
	}
}

function reset_wm_win_lose_btn(customer_id){
	requestJSON("cus_info_editor_op", "pdisplay=reset_wm_win_lose", "customer_id=" + customer_id);
}

function request_bank_ver_div(edit_info_id){
	requestJSON("cus_info_editor_op", "pdisplay=request_bank_ver_div", "edit_info_id=" + edit_info_id);
}
...
function check_bank_ver_div(){
	...
	bank_ver_div_change_bank_area($('#bank-ver-div #bank-area-select').val());
	show_bank_ver_div();
}
...
function save_bank_ver(bank_ver_status){
	var post_ele = $("#bank-ver-div [is-post-data=1]").serialize();
	requestJSON("cus_info_editor_op", "pdisplay=save_bank_ver", post_ele);
}
