    ...
    var handleRecords = function () {
        grid.init({
            ...
            dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
                ...
                "ajax": {
                    "url": "/op/payment_company_manager_op.php?pdisplay=display_manager_list&select_status=" + $("select[name=select_status]").val(), // ajax source
...
$(function(){
	...
	$(".search-btn").click(function(){
		grid.getDataTable().ajax.url("/op/payment_company_manager_op.php?pdisplay=display_manager_list&select_status=" + $("select[name=select_status]").val()).load();
	});
});

function add_payment_company(){
	location.href = "payment_company_editor.php?etype=add";
}

function initialize_quota(ps_id){
	if(confirm(change_lang_txt({"org_txt" : "確定要清零"}) + "?")){
		requestJSON("payment_company_manager_op.php", "pdisplay=initialize_quota", "ps_id=" + ps_id);
	}
}