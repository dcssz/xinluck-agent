    ...
    var handleRecords = function () {
        grid.init({
            ...
            dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
                ...
                "ajax": {
                    "url": "/op/discount_category_op.php?pdisplay=display_manager_list", // ajax source
                },
				 "bSort": false,
                /*"order": [
                    [1, "asc"]
...
function request_editor_item_div(etype, edit_cid){
	requestJSON("discount_category_op.php", "pdisplay=request_editor_item_div", "etype=" + etype + "&edit_cid=" + edit_cid);	
}
...
function save_discount_category(){
	var etype = $("#etype").val();
	var edit_cid = $("#edit-cid").val();
	
    var category_name_count = 0;
	...
	if(category_name_count == 0)
		pop_msg(change_lang_txt({"org_txt" : "請輸入優惠分類名稱"}) + "!");
	else{
        requestJSON("discount_category_op.php", "pdisplay=save_discount_category", "etype=" + etype + "&edit_cid=" + edit_cid, "#manager-editor");   
    }
}

function delete_item(edit_cid){
	if(confirm(change_lang_txt({"org_txt" : "確定要刪除"}) + "?")){
		requestJSON("discount_category_op.php", "pdisplay=display_delete_item", "edit_cid=" + edit_cid);
	}
}
