...
$(function(){
	...
	$(".search-btn").click(function(){
		//grid.getDataTable().ajax.url("/op/company_deposit_orders_op.php?pdisplay=display_manager_list&search_customer_userid=" + $("input[name=search_customer_userid]").val() + "&fuzzy_search=" + ($("input[name=fuzzy_search]").prop("checked") ? "1" : "0") + "&search_status=" + $("select[name=search_status]").val() + "&search_order_no=" + $("input[name=search_order_no]").val() + "&search_start_date=" + $('#search-start-date').val() + "&search_start_time=" + $('#search-start-time').val() + "&search_end_date=" + $('#search-end-date').val() + "&search_end_time=" + $('#search-end-time').val()).load();
		grid.getDataTable().ajax.url("/op/company_deposit_orders_op.php?pdisplay=display_manager_list&" + $('#report-form').serialize()).load();
	});
});
...
function request_editor_item_div(edit_order_id){
	requestJSON("company_deposit_orders_op.php", "pdisplay=request_editor_item_div", "edit_order_id=" + edit_order_id);
}
...
function save_audit(){
	...
	requestJSON("company_deposit_orders_op.php", "pdisplay=save_audit", "edit_order_id=" + edit_order_id, "#editor-form");
}

function request_order_log_div(edit_order_id){
	requestJSON("company_deposit_orders_op.php", "pdisplay=request_order_log_div", "edit_order_id=" + edit_order_id);
}
...
function locked_orders(edit_order_id){
	requestJSON("company_deposit_orders_op.php", "pdisplay=locked_orders", "edit_order_id=" + edit_order_id);
}
