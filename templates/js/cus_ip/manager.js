    ...
    var handleRecords = function () {
        grid.init({
            ...
            dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
                ...
                "ajax": {
                    "url": "/op/cus_ip_op.php?pdisplay=display_manager_list", // ajax source
                },
				 "bSort": false,
                /*"order": [
                    [1, "asc"]
...
$(function(){
	...
	$(".search-btn").click(function(){
		grid.getDataTable().ajax.url("/op/cus_ip_op.php?pdisplay=display_manager_list&search_customer_userid=" + $("input[name=search_customer_userid]").val() + "&search_login_ip=" + $("input[name=search_login_ip]").val() + "&select_status=" + $("select[name=select_status]").val()).load();
	});
});

function change_status(edit_info_id, edit_status){
	requestJSON("cus_ip_op.php", "pdisplay=change_status", "edit_info_id=" + edit_info_id + "&edit_status=" + edit_status);
}