    ...
    var handleRecords = function () {
        grid.init({
            ...
            dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
                ...
                "ajax": {
                    "url": "/op/cus_retreat_audit_op.php?pdisplay=display_manager_list&search_customer_userid=" + $("input[name=search_customer_userid]").val() + "&fuzzy_search=" + ($("input[name=fuzzy_search]").prop("checked") ? "1" : "0"), // ajax source
                },
				 "bSort": false,
                /*"order": [
                    [1, "asc"]
...
$(function(){
	...
	$(".search-btn").click(function(){
		grid.getDataTable().ajax.url("/op/cus_retreat_audit_op.php?pdisplay=display_manager_list&search_customer_userid=" + $("input[name=search_customer_userid]").val() + "&fuzzy_search=" + ($("input[name=fuzzy_search]").prop("checked") ? "1" : "0") + "&search_audit_status=" + $("select[name=search_audit_status]").val()).load();
	});
});

function request_editor_item_div(etype, edit_audit_id){
	requestJSON("cus_retreat_audit_op.php", "pdisplay=request_editor_item_div", "etype=" + etype + "&edit_audit_id=" + edit_audit_id);	
}
...
function save_audit(){
	...
	requestJSON("cus_retreat_audit_op.php", "pdisplay=save_audit", "etype=" + etype + "&edit_audit_id=" + edit_audit_id + "&edit_audit_status=" + edit_audit_status);
}
