    ...
    var handleRecords = function () {
        grid.init({
            ...
            dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
                ...
                "ajax": {
                    "url": "/op/extra_commission_rule_op.php?pdisplay=display_manager_list&edit_unique_code=" + $("#edit-unique-code").val(), // ajax source
                },
				 "bSort": false,
                /*"order": [
                    [1, "asc"]
...
function request_editor_item_div(etype, edit_extra_commission_rule_id){
	var edit_unique_code = $("#edit-unique-code").val();
	requestJSON("extra_commission_rule_op.php", "pdisplay=request_editor_item_div", "etype=" + etype + "&edit_extra_commission_rule_id=" + edit_extra_commission_rule_id + "&edit_unique_code=" + edit_unique_code);	
}
...
function add_param_item(){
	...
	$("#editor-item-div .station-param-tb").append('<tr item_list_index="item" name="extra_commission_condition">' + add_content + '</tr>');
...
function save_extra_commission_rule(){
	var etype = $("#etype").val();
	var edit_extra_commission_rule_id = $("#edit-extra-commission-rule-id").val();
	var extra_commission_rule_name = $("input[name=extra_commission_rule_name]").val();
	var edit_unique_code = $("#edit-unique-code").val();
	
	if(typeof extra_commission_rule_name == 'undefined')
		extra_commission_rule_name = "";
	
	if(extra_commission_rule_name == "")
		pop_msg(change_lang_txt({"org_txt" : "請輸入規則名稱"}) + "!");
	else
		requestJSON("extra_commission_rule_op.php", "pdisplay=save_extra_commission_rule", "etype=" + etype + "&edit_extra_commission_rule_id=" + edit_extra_commission_rule_id + "&edit_unique_code=" + edit_unique_code, "#editor-item-form");
}

function delete_item(edit_extra_commission_rule_id){
	if(confirm(change_lang_txt({"org_txt" : "確定要刪除"}) + "?")){
		requestJSON("extra_commission_rule_op.php", "pdisplay=display_delete_item", "edit_extra_commission_rule_id=" + edit_extra_commission_rule_id);
	}
}