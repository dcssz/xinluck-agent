    ...
    var handleRecords = function () {
        grid.init({
            ...
            dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
                ...
                "ajax": {
                    "type":"GET",
                    "url": "/agent/self_quota_log_op?pdisplay=display_manager_list&search_customer_userid=" + $("input[name=search_customer_userid]").val() + "&fuzzy_search=" + ($("input[name=fuzzy_search]").prop("checked") ? "1" : "0"), // ajax source
                },
				 "bSort": false,
                /*"order": [
                    [1, "asc"]
...
$(function(){
	...
	$(".search-btn").click(function(){
		grid.getDataTable().ajax.url("/agent/self_quota_log_op?pdisplay=display_manager_list&search_customer_userid=" + $("input[name=search_customer_userid]").val() + "&fuzzy_search=" + ($("input[name=fuzzy_search]").prop("checked") ? "1" : "0") + "&search_operate_type=" + $("select[name=search_operate_type]").val() + "&search_trans_type=" + $("select[name=search_trans_type]").val()).load();
	});
});
